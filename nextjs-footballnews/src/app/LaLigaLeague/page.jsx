...
async function fetchMatches(season) {
  const response = await fetch(`http://localhost:3000/api/LaLiga?season=${season}`);
  if (!response.ok) {
...
export default function LaLigaLeaguePage() {
  ...
  return (
    <div>
      ...
      <div className="container mx-auto p-4 pt-6 mt-10 font-medium">
        <h1 className="text-2xl font-bold mb-4 text-center">ลา ลีกา {season}</h1>
